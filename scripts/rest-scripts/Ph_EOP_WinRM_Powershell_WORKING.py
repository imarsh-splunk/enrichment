def GetMessageTrace_find_recipients(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None):
    ...
    container_data = phantom.collect2(container=container, datapath=['filtered-data:filter_external_email:condition_1:artifact:*.cef.fromEmail', 'filtered-data:filter_external_email:condition_1:artifact:*.cef.emailSubject'])
...
$pw = ConvertTo-SecureString '86Krispydoughnut' -AsPlainText -Force
...
foreach($item in $trace){{
    ...
    if ($recipients.RecipientType -notlike 'UserMailbox'){{
        ...
        Get-DistributionGroupMember -Identity $recipients.PrimarySmtpAddress
...
""".format(sender, subject),
    ...
    phantom.act("run script", parameters=parameters, assets=['win10vm_homelab'], name="GetMessageTrace_find_recipients")
