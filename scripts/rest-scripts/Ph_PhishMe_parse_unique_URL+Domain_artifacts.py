def parse_unique_artifacts_from_body(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None):
    phantom.debug('parse_unique_artifacts_from_body() called')
    ...
    for url in unique_urls:
        if 'protect2.fireeye.com' in url:
            ...
            phantom.debug('url after: {}'.format(url_parsed))
            
            # Add only unique and filtered URLs into the single-key CEF dictionary
            cef['requestURL'] = url_parsed
            
            # Add a new URL Artifact for each URL found
            phantom.add_artifact(
            container=container, raw_data=raw, cef_data=cef, label='url',
            name='URL Artifact', severity='medium', identifier=None, artifact_type='url')
    ...
    for domain in unique_domains:
        if 'protect2.fireeye.com' not in domain:
            ...
            phantom.add_artifact(
            container=container, raw_data=raw, cef_data=cef, label='domain',
            name='Domain Artifact', severity='medium', identifier=None, artifact_type='domain')
