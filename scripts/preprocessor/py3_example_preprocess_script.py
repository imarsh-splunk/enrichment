...
def exclude_domains(artifact):
    ...
    supressed_domains = ['esl.org', 'www.proofpoint.com', 'urldefense.com','urldefense.proofpoint.com','lists.fsisac.com']
...
def exclude_urls(artifact):
    ...
    supressed_characters = ['[', ']']

    if artifact['name'] == 'URL Artifact':
        if any(char in artifact['cef'].get('requestURL', '') for char in supressed_characters):
            return False
    return True
...
def preprocess_container(container):
    ...
    try:
        ...
        artifacts = list(filter(exclude_urls, artifacts))
