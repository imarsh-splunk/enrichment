...
def exclude_urls(artifact):
    ...
    suppressed_hosts = ['#', 'www.virustotal.com']
    suppressed_hosts = ['#']
    ...
    if artifact.get('name') == 'URL Artifact':
        ...
        if url in suppressed_hosts:
            return False

        parsed = urlparse.urlsplit(url)
        host = parsed[1]

        if host in suppressed_hosts:
            return False
...
def exclude_domains(artifact):
    ...
    suppressed_domains = ['www.virustotal.com', 'gsk.com', 'GSK.COM', 'urldefense.com', 'www.microsoft.com',
    ...
    if artifact.get('name') == 'Domain Artifact':
        if artifact['cef'].get('destinationDnsDomain') in suppressed_domains:
            return False

    return True

def clean_domain_artifacts(artifact):
    if artifact.get('name') == 'Domain Artifact':
        if artifact['cef'].get('destinationDnsDomain'):
            return {'name': 'Domain Artifact', 'cef':{'destinationDnsDomain': artifact['cef'].get('destinationDnsDomain')}}
...
def preprocess_container(container):
            ...
            if requesturl:
                ...
                if newurl:
                    if newurl.startswith("domain:"):
                        ...
                        artifact = {'name': 'Domain Artifact', 'cef':{'destinationDnsDomain': domain}}
    ...
    # clean out domain artifacts fields
    new_artifacts = map(clean_domain_artifacts, new_artifacts)
