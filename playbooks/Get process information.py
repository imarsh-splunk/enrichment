"""
This playbook uses destination hostnames and process names to gathers process and parent process information regarding the notable and places them  in a note within Task - Process analysis.
"""
...
def host_filter(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    ...
    if matched_artifacts_1 or matched_results_1:
        merge_dedup_dest(action=action, success=success, container=container, results=results, handle=handle, custom_function=custom_function, filtered_artifacts=matched_artifacts_1, filtered_results=matched_results_1)
...
def parent_process_notes(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    ...
    formatted_data_2 = phantom.get_format_data(name='parent_process_info_results')
    ...
    if formatted_data_1 and formatted_data_2:
        ...
        content = formatted_data_1 + formatted_data_2
    else:
...
%%"""
    ...
    parameters = [
        ...
        "merge_dedup_dest:custom_function_result.data.*.item",
    ]
...
def merge_dedup_dest(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    phantom.debug('merge_dedup_dest() called')
    
    filtered_artifacts_data_0 = phantom.collect2(container=container, datapath=['filtered-data:host_filter:condition_1:artifact:*.cef.destinationHostName', 'filtered-data:host_filter:condition_1:artifact:*.cef.dest', 'filtered-data:host_filter:condition_1:artifact:*.cef.dest_nt_host', 'filtered-data:host_filter:condition_1:artifact:*.cef.sourceHostName', 'filtered-data:host_filter:condition_1:artifact:*.cef.src', 'filtered-data:host_filter:condition_1:artifact:*.cef.src_nt_host', 'filtered-data:host_filter:condition_1:artifact:*.cef.deviceHostname', 'filtered-data:host_filter:condition_1:artifact:*.cef.dvc', 'filtered-data:host_filter:condition_1:artifact:*.cef.dvc_nt_host'])
    ...
    phantom.custom_function(custom_function='ps-playbooks-dev/list_merge_dedup', parameters=parameters, name='merge_dedup_dest', callback=get_process_info_spl)
